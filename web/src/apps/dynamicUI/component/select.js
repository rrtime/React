...
export default class SelectComponent extends React.Component {
	...
	calculateState(props){
		let {data} = this.state,
			...
			isFocus =  _isFocus || _getter(_path, 'isFocus') || false,
			dataSourceFetchMode = _getter(_path, 'dataSourceFetchMode'),
			dataSource = _getter(_path, 'dataSource'),
			displayMember = _getter(_path, 'displayMember') || 'name',
      		valueMember = _getter(_path, 'valueMember') || 'code',
			width = _getter(_path,'width')
		this.oldValue = value
	...
	getOptions(){
		...
		return dataSource ? dataSource.map((item,index)=>
			<Option key={item.get(this.get('valueMember'))}>
